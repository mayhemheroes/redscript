...
fn compile_overloaded_call() {
    ...
    let check = check_code![
        ...
        mem!(Local(val)),
        mem!(New(class)),
        pat!(InvokeStatic(Offset { value: 35 }, 1, _, 0)),
        pat!(RefToWeakRef),
        mem!(Local(val)),
        mem!(Local(val)),
        pat!(ParamEnd),
...
fn compile_variant_intrinsics() {
    ...
    let check = check_code![
        ...
        pat!(Assign),
        mem!(Local(b)),
        pat!(InvokeStatic(Offset { value: 36 }, 1, _, 0)),
...
fn compile_implicit_conversions() {
    ...
    let check = check_code![
        ...
        pat!(WeakRefToRef),
        mem!(Local(a)),
        pat!(InvokeVirtual(Offset { value: 38 }, 1, _, 0)),
...
fn compile_string_interpolation() {
    ...
    let check = check_code![
        ...
        pat!(StringConst(_)),
        pat!(Return),
        pat!(InvokeStatic(Offset { value: 203 }, 1, _, 3)),
        mem!(AsRef(str_type)),
        pat!(InvokeStatic(Offset { value: 87 }, 1, _, 2)),
        mem!(AsRef(str_type)),
        pat!(StringConst(_)),
        mem!(AsRef(str_type)),
        pat!(InvokeStatic(Offset { value: 48 }, 1, _, 0)),
        mem!(AsRef(str_type)),
        mem!(Local(name)),
        mem!(AsRef(str_type)),
        pat!(StringConst(_)),
        ...
        pat!(ParamEnd),
        mem!(AsRef(str_type)),
        pat!(InvokeStatic(Offset { value: 82 }, 1, _, 1)),
        mem!(AsRef(str_type)),
        pat!(ToString(_)),
        pat!(InvokeStatic(Offset { value: 34 }, 1, _, 0)),
        mem!(Param(year)),
